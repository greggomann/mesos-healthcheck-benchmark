...
while (( $LEFT < $RIGHT )); do
  ...
  /opt/mesosphere/active/mesos/bin/mesos-execute --principal=mesos-execute --master=leader.mesos:5050 --secret="${DCOS_SERVICE_ACCOUNT_CREDENTIAL}" --task_group="file://$PWD/task-group-$NUM_TASKS.json" &>>$LOG_FILE&
